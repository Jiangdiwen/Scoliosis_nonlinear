...
def nochangeCalc_SSMTsfm_2PtSets(Vy, SSM, deform_TH=0, PartialList=0, numPC=0, ind_PCs=0, Sz_Target=0,
    ...
    # the fitted points
    Vx0 = SSM['Verts_mean'] + np.dot(coef, SSM['Eigenvect'][ind_PCs, :])
    Vx0 = np.reshape(Vx0, (num_Verts1, 3))
    if noSimTsfm == 0:
        Vx = np.hstack((Vx0, np.zeros((num_Verts1, 1)) + 1))
        Vx = np.dot(Vx, Sim)
        Vx = Vx[:, 0:3]
    else:
        Vx = Vx0
