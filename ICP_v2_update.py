...
def Calc_SSMTsfm_2PtSets(Vy, SSM, deform_TH=0, PartialList=0, numPC=0, ind_PCs=0, Sz_Target=0,
    ...
    num_Verts1 = int(np.size(SSM['Verts_mean']) / 3)
    ...
    if PartialList == 0:
        if noSimTsfm == 0:
            Vx_mean = np.reshape(SSM['Verts_mean'], (num_Verts, 3))  # mean shape of the SSM
        ...
        a = SSM['Eigenvect'][ind_PCs, :].T #这里为啥要转置呢
        
        Vy_norm = Vy_norm.flatten()
        b = Vy_norm - SSM['Verts_mean']
        ...
        if noSimTsfm == 0:
            Vx_mean = np.reshape(SSM['Verts_mean'][ind_fit],(nv_Partial, 3))
        ...
        # deform model
        a = SSM['Eigenvect'][ind_PCs, :]
        a = a[:, ind_fit].T
        Vy_norm = Vy_norm.flatten()
        b = Vy_norm - SSM['Verts_mean'][ind_fit]
    ...
    # the fitted points
    ind_PCs_total =  np.arange(187800)
    # print('coef.shape = ', coef.shape, '\nVerts_mean_total.shape = ', SSM['Verts_mean_total'].shape
    # , '\nEigenvect.shape = ', SSM['Eigenvect_total'][ind_PCs_total, :].shape, '\ind_PCs_total = ', ind_PCs_total)
    # print('coef=',coef)
    
    
    Vx0 = SSM['Verts_mean_total'] + np.dot(coef, SSM['Eigenvect_total'][ind_PCs_total, :])
    Vx0 = np.reshape(Vx0, (62600, 3))
    if noSimTsfm == 0:
        Vx = np.hstack((Vx0, np.zeros((62600, 1)) + 1))
        Vx = np.dot(Vx, Sim)
        Vx = Vx[:, 0:3]
    else:   
        Vx = Vx0
