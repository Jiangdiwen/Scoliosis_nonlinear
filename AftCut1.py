'''
Author: Yanxin_Jiang
Date: 2022-06-20 19:37:07
LastEditors: Yanxin_Jiang
LastEditTime: 2022-07-13 16:15:35
Description: 
...
def SSM_TPS(Total_verts,target_file):
    ...
    #-----降采样加快TPS速度
    Total_verts_down = Total_verts[:: 5]    
    ...
    #-----
    x, Suf_verts_TPS = warp_TPS(Suf_verts_down, Total_verts, target_verts) #x是降采样后TPS结果，result是降采样前TPS结果
...
if __name__ =='__main__':
    ...
    Skin_vertsSuf, Skin_triSuf,Py_TRISuf = GetPtsFromSSM(Skin_file)
    ...
    # 输入参数设置
    TPS_input1 = mesh_Mode.points.copy()
    ...
    while(j < trial and mean_d > thred):
        ...
        #-----1.SSM体表与病人体表做TPSRPM-----------
        Skin_TPS = SSM_TPS(TPS_input1, TPS_input2)
        time.sleep(2)           
        # a = TPS_input1.copy()  
        # a = a[:,[0,2,1]]
        # a = a[:,[1,0,2]]
        ...
        # a[:,2] = -a[:,2]
                ...
                p.add_mesh(mesh_patient, color="mintcream", show_edges=True, opacity=1.0)
                p.add_mesh(Skin_TPS, color="green", show_edges=True, opacity=1.0)
                p.show()
        ...
        X0_2Dpoint, Y0_2Dpoint, ConePrj_img = ConePrj_Mesh(SSM1_verts, Skin_triSuf, Dtctr_in_world, 
